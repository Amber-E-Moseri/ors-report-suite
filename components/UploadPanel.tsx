...
import { validateFTHeaders, validateNotesHeaders, validateGroupHeaders } from '@/lib/csv-parser'
...
export function UploadPanel({ onReady, hasStoredGroups, storedGroupCount }: Props) {
  ...
  async function handleFile(
    ...
    try {
      ...
      setSlot({ file, error: (e as Error).message, ok: false })
    }
  }
  ...
  function onDrop(
    ...
    if (file) handleFile(file, setSlot, validator)
  }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      ...
      <div className="mb-10 text-center">
        ...
        >
          <span className="text-white font-bold text-2xl tracking-tight">ORS Report Suite</span>
        </div>
        <p className="text-slate-500 text-sm mt-1">Upload your CSV exports to generate ORS follow-up and first-timer reports</p>
      </div>
      ...
      <div className="w-full max-w-3xl grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <DropZone
          ...
          state={notes}
          inputRef={notesRef}
          onFileSelect={f => handleFile(f, setNotes, validateNotesHeaders)}
        ...
        <DropZone
          ...
          state={cellFT}
          inputRef={cellRef}
          onFileSelect={f => handleFile(f, setCellFT, validateFTHeaders)}
        ...
        <DropZone
          ...
          state={svcFT}
          inputRef={svcRef}
          onFileSelect={f => handleFile(f, setSvcFT, validateFTHeaders)}
          onDrop={e => onDrop(e, setSvcFT, validateFTHeaders)}
        />
        <DropZone
          label="GROUP Sheet"
          description={hasStoredGroups ? `${storedGroupCount} groups stored — upload to update` : "Group directory CSV (optional)"}
          state={groupF}
          inputRef={groupRef}
          onFileSelect={f => handleFile(f, setGroupF, validateGroupHeaders)}
          onDrop={e => onDrop(e, setGroupF, validateGroupHeaders)}
        />
      </div>

      {/* Stored groups notice */}
      {hasStoredGroups && (
        <div className="w-full max-w-3xl mb-4 px-4 py-2.5 rounded-lg text-xs font-medium flex items-center gap-2"
          style={{ background: '#D9F2E3', color: '#1B5E3C' }}>
          ✓ {storedGroupCount} groups loaded from saved directory — GROUP sheet upload is optional this session.
        </div>
      )}

      {/* Requirements note */}
      <p className="text-xs text-slate-400 mb-6 text-center max-w-md">
        <strong>Required:</strong> ELVANTO Notes. &nbsp;
        <strong>Optional but recommended:</strong> Cell FTs, Service FTs, and GROUP sheet for complete reports.
      </p>
...
function DropZone({ label, description, required, state, inputRef, onFileSelect, onDrop }: DropZoneProps) {
    ...
    >
      ...
      <div className="flex items-start gap-3">
        <div className="mt-0.5">
          {state.ok    ? <CheckCircle  size={20} color="#22C55E" /> :
           state.error ? <AlertCircle  size={20} color="#EF4444" /> :
