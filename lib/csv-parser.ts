...
export function validateGroupHeaders(rows: Record<string, unknown>[]): { ok: boolean; missing: string[] } {
  ...
  const hasGroupName = headers.some(h =>
    h === 'name' ||
    h === 'group_name' ||
    h === 'group name'
  )
  const hasSubgroup = headers.some(h =>
    h.includes('categor') ||
    h === 'subgroup' ||
    h === 'leadership category' ||
    h === 'leadership_category'
  )
  if (!hasGroupName) missing.push('name / group_name')
  if (!hasSubgroup) missing.push('categories / subgroup')
