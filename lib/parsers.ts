...
export function parseGroupRows(rows: Record<string, unknown>[]): GroupDirectory {
  ...
  function getValue(row: Record<string, unknown>, keys: string[]): unknown {
    for (const key of keys) {
      const found = Object.entries(row).find(([k]) => k.toLowerCase().trim() === key)
      if (found) return found[1]
    }
    return undefined
  }

  function normalizeSubgroup(input: string): FollowUpSubgroup | null {
    const direct = CATEGORY_MAP[input]
    if (direct) return direct

    const lower = input.toLowerCase().replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim()
    const hasA = /\bsga\b|subgroup a/.test(lower)
    const hasB = /\bsgb\b|subgroup b/.test(lower)
    const east = lower.includes('central east') || lower.includes('central-east')
    const central = lower.includes('central')
    const west = lower.includes('west')

    if (east && hasA) return 'BLW Canada Central-East SGA'
    if (east && hasB) return 'BLW Canada Central-East SGB'
    if (west && hasA) return 'BLW Canada West SGA'
    if (west && hasB) return 'BLW Canada West SGB'
    if (central && !east && hasA) return 'BLW Canada Central SGA'
    if (central && !east && hasB) return 'BLW Canada Central SGB'

    return null
  }

  for (const row of rows) {
    const nm  = str(getValue(row, ['name', 'group_name', 'group name']))
    const cat = str(getValue(row, ['categories', 'category', 'subgroup', 'leadership category', 'leadership_category']))
    const mem = toNum(getValue(row, ['total members', 'members', 'total_members']))
    if (!nm) continue

    const subgroup = normalizeSubgroup(cat)
...
export function buildAllFTData(
  ...
  const cellByName = new Map<string, ParsedFTRow>()
  for (const row of cellData.rows) {
    const key = row.fullName.trim().toLowerCase()
    if (key) cellByName.set(key, row)
  }

  const conflicts: FTConflictRow[] = []
  const conflictKeys = new Set<string>()
  for (const row of serviceData.rows) {
    const key = row.fullName.trim().toLowerCase()
    if (!key) continue
    const cellRow = cellByName.get(key)
    if (cellRow && !conflictKeys.has(key)) {
      conflictKeys.add(key)
      conflicts.push({
        fullName: cellRow.fullName,
        cellGroup: cellRow.group,
        cellSubgroup: cellRow.subgroup,
        serviceGroup: row.group,
        serviceSubgroup: row.subgroup,
      })
    }
  }
